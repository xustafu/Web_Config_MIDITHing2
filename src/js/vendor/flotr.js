...
!(function (name, context, definition) {
  ...
  var win = window,
    ...
    fixEvent = (function () {
      var commonProps =
          'altKey attrChange attrName bubbles cancelable ctrlKey currentTarget detail eventPhase getModifierState isTrusted metaKey relatedNode relatedTarget shiftKey srcElement target timeStamp type view which'.split(
            ' '
          ),
        mouseProps = commonProps.concat(
          'button buttons clientX clientY dataTransfer fromElement offsetX offsetY pageX pageY screenX screenY toElement'.split(
            ' '
          )
        ),
      ...
      return function (event, isNative) {
        ...
        if (isNative) {
          // we only need basic augmentation on custom events, the rest is too expensive
          if (type.indexOf('key') !== -1) {
            ...
            if (overOut.test(type))
              result.relatedTarget =
                event.relatedTarget || event[(type === 'mouseover' ? 'from' : 'to') + 'Element'];
    ...
    nativeHandler = function (element, fn, args) {
      return function (event) {
        event = fixEvent(
          event || ((this.ownerDocument || this.document || this).parentWindow || win).event,
          true
        );
      ...
      return function (event) {
        if (
          condition
            ...
            : W3C_MODEL
            ? true
            : (event && event.propertyName === '_on' + type) || !event
        ) {
          if (event)
            event = fixEvent(
              event || ((this.ownerDocument || this.document || this).parentWindow || win).event,
          ...
          fn.apply(
            element,
            event && (!args || args.length === 0)
              ? arguments
              : slice.call(arguments, event ? 0 : 1).concat(args)
          );
        }
      };
    },
    ...
    addListener = function (element, orgType, fn, originalFn, args) {
      ...
      if (registry.has(element, type, fn)) return element; // no dupe
      ...
      entry = registry.put(
        new RegEntry(element, type, fn, originalFn, namespaces[0] && namespaces)
      );
    ...
    del = function (selector, fn, $) {
      return function (e) {
        var target,
          i,
          array = typeof selector === 'string' ? $(selector, this) : selector;
...
Flotr.defaultOptions = {
  colors: ['#FFFFFF', '#C0D800', '#CB4B4B', '#4DA74D', '#9440ED'], //=> The default colorscheme. When there are > 5 series, additional colors are generated.
  ieBackgroundColor: '#252525', // Background color for excanvas clipping
  ...
  xaxis: {
    ...
    showLabels: false, // => setting to true will show the axis ticks labels, hide otherwise
  ...
  grid: {
    color: '#676767', // => primary color used for outline and labels
...
(function () {
  ...
  Color.prototype = {
    ...
    contrast: function () {
      var test = 1 - (0.299 * this.r + 0.587 * this.g + 0.114 * this.b) / 255;
  ...
  _.extend(Color, {
    ...
    parse: function (color) {
      ...
      return (result = COLOR_NAMES[name])
        ? new Color(result[0], result[1], result[2])
        : new Color(0, 0, 0, 0);
    },
    ...
    processColor: function (color, options) {
      ...
      if (!options.ctx) {
        ...
        return Color.parse(_.isArray(grad.colors[0]) ? grad.colors[0][1] : grad.colors[0])
          .alpha(opacity)
          .toString();
      }
      ...
      var i,
        c,
        stop,
        gradient = options.ctx.createLinearGradient(options.x1, options.y1, options.x2, options.y2);
...
Flotr.Date = {
  ...
  generator: function (axis) {
    ...
    formatter =
      options.tickFormatter === Flotr.defaultTickFormatter ? this.formatter : options.tickFormatter;
...
(function () {
  ...
  Text.prototype = {
    ...
    html: function (text, element, style, className) {
      ...
      D.insert(
        div,
        '<div style="' + style + '" class="' + className + ' flotr-dummy-div">' + text + '</div>'
      );
...
(function () {
  ...
  Graph.prototype = {
    ...
    extendRange: function (type, series) {
      if (this[type].extendRange)
        this[type].extendRange(series, series.data, series[type], this[type]);
      if (this[type].extendYRange)
        this[type].extendYRange(series.yaxis, series.data, series[type], this[type]);
      if (this[type].extendXRange)
        this[type].extendXRange(series.xaxis, series.data, series[type], this[type]);
    },
    ...
    calculateSpacing: function () {
      ...
      if (x.options.margin === false) {
        ...
        p.bottom +=
          (options.grid.circular
            ? 0
            : x.used && x.options.showLabels
            ? x.maxLabel.height + margin
            : 0) +
        ...
        p.top +=
          (options.grid.circular
            ? 0
            : x2.used && x2.options.showLabels
            ? x2.maxLabel.height + margin
            : 0) +
          (x2.used && x2.options.title ? x2.titleSize.height + margin : 0) +
          this.subtitleHeight +
          this.titleHeight +
          maxOutset;
      } else {
      ...
      if (y.options.margin === false) {
        ...
        p.left +=
          (options.grid.circular
            ? 0
            : y.used && y.options.showLabels
            ? y.maxLabel.width + margin
            : 0) +
          (y.used && y.options.title ? y.titleSize.width + margin : 0) +
          maxOutset;

        p.right +=
          (options.grid.circular
            ? 0
            : y2.used && y2.options.showLabels
            ? y2.maxLabel.width + margin
            : 0) +
          (y2.used && y2.options.title ? y2.titleSize.width + margin : 0) +
          maxOutset;
      } else {
    ...
    */
    ...
    drawTooltip: function (content, x, y, options) {
      var mt = this.getMouseTrack(),
        style =
          'opacity:0.7;background-color:#000;color:#fff;display:none;position:absolute;padding:2px 8px;-moz-border-radius:4px;border-radius:4px;white-space:nowrap;',
        p = options.position,
        m = options.margin,
        plotOffset = this.plotOffset;
      ...
      if (x !== null && y !== null) {
        if (!options.relative) {
          // absolute to the canvas
          if (p.charAt(0) == 'n') style += 'top:' + (m + plotOffset.top) + 'px;bottom:auto;';
          else if (p.charAt(0) == 's')
            style += 'bottom:' + (m + plotOffset.bottom) + 'px;top:auto;';
    ...
    _initEvents: function () {
      ...
      if ('ontouchstart' in el) {
        ...
        this.observe(this.overlay, 'mousedown', _.bind(this.mouseDownHandler, this))
    ...
    _initCanvas: function () {
      ...
      size.height = el.parentElement.clientHeight;
    ...
    _initOptions: function (opts) {
      ...
      if (
        this.options.grid.minorVerticalLines === null &&
        this.options.xaxis.scaling === 'logarithmic'
      ) {
        this.options.grid.minorVerticalLines = true;
      }
      if (
        this.options.grid.minorHorizontalLines === null &&
