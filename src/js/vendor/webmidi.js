(function (scope) {
  ...
  WebMidi.prototype.enable = function (callback, sysex) {
    ...
    if (!this.supported) {
      if (typeof callback === 'function') {
        callback(
          new Error('The Web MIDI API is not supported by your browser.')
        );
      }
  ...
  WebMidi.prototype.getOctave = function (number) {
    if (number != null && number >= 0 && number <= 127) {
      return (
        Math.floor(Math.floor(number) / 12 - 1) + Math.floor(wm.octaveOffset)
      );
    }
  };
  ...
  Input.prototype._parseNrpnEvent = function (e) {
    ...
    if (
      !(
        ...
        ((data1 >= wm.MIDI_NRPN_MESSAGES.increment &&
          data1 <= wm.MIDI_NRPN_MESSAGES.parammsb) ||
    ...
    if (
      ...
      wm._nrpnBuffer[channelBufferIndex][2].number ===
        wm.MIDI_NRPN_MESSAGES.entrymsb &&
      ...
      var nrpnNumber =
        (wm._nrpnBuffer[channelBufferIndex][0].value << 7) |
      ...
      if (wm._nrpnBuffer[channelBufferIndex].length === 6) {
        nrpnValue =
          (wm._nrpnBuffer[channelBufferIndex][2].value << 7) |
  ...
  Input.prototype._parseChannelEvent = function (e) {
    ...
    if (
      ...
      this._userHandlers.channel[event.type][channel].forEach(function (
        callback
      ) {
        callback(event);
      });
  ...
  Input.prototype._parseSystemEvent = function (e) {
    ...
    if (this._userHandlers.system[event.type]) {
      this._userHandlers.system[event.type].forEach(function (callback) {
        callback(event);
      });
    }
  };
  ...
  Output.prototype.stopNote = function (note, channel, options) {
    ...
    this._convertNoteToArray(note).forEach(
      function (item) {
        wm.toMIDIChannels(channel).forEach(
          function (ch) {
            this.send(
   ...
   * @return {Output} Returns the `Output` object so methods can be chained.
   */
  Output.prototype.sendKeyAftertouch = function (
    note,
    channel,
    pressure,
    options
   ...
   * @return {Output} Returns the `Output` object so methods can be chained.
   */
  Output.prototype.sendControlChange = function (
    controller,
    value,
    channel,
    options
   ...
   * @returns {Output}
   */
  Output.prototype._selectRegisteredParameter = function (
    parameter,
    channel,
    time
  ) {
   ...
   * @returns {Output}
   */
  Output.prototype._selectNonRegisteredParameter = function (
    parameter,
    channel,
    time
  ) {
   ...
   * @returns {Output}
   */
  Output.prototype._setCurrentRegisteredParameter = function (
    data,
    channel,
    time
  ) {
   ...
   */
  Output.prototype.setRegisteredParameter = function (
    parameter,
    data,
    channel,
    options
   ...
   * @returns {Output} Returns the `Output` object so methods can be chained.
   */
  Output.prototype.setNonRegisteredParameter = function (
    parameter,
    data,
    channel,
    options
   ...
   * @returns {Output} Returns the `Output` object so methods can be chained.
   */
  Output.prototype.incrementRegisteredParameter = function (
    parameter,
    channel,
    options
  ) {
   ...
   * @returns {Output} Returns the `Output` object so methods can be chained.
   */
  Output.prototype.decrementRegisteredParameter = function (
    parameter,
    channel,
    options
  ) {
   ...
   * @return {Output} Returns the `Output` object so methods can be chained.
   */
  Output.prototype.setPitchBendRange = function (
    semitones,
    cents,
    channel,
    options
   ...
   * @return {Output} Returns the `Output` object so methods can be chained.
   */
  Output.prototype.setModulationRange = function (
    semitones,
    cents,
    channel,
    options
   ...
   *
   */
  Output.prototype.sendChannelMode = function (
    command,
    value,
    channel,
    options
   ...
   * @return {Output} Returns the `Output` object so methods can be chained.
   */
  Output.prototype.sendChannelAftertouch = function (
    pressure,
    channel,
    options
  ) {
